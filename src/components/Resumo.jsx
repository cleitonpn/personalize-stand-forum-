...
export default function Resumo() {
  ...
  const paredeCor = corPorId(state.parede.corId)
  const madeiraCor = corPorId(state.napaMadeira.corId)

  const briefing = () => construirBriefing(state, orcamento, { pisoCor, paredeCor, madeiraCor })

  const exportarPDF = () => {
    const w = window.open('', '_blank')
    if (!w) { alert('Permita pop-ups para gerar o briefing.'); return }
    w.document.write(briefingHTML(briefing()))
    w.document.close()
    w.focus()
    ...
    <div>
      ...
      <div className="orc-list">
        ...
        {orcamento.linhas.length === 0 && (
          <div className="orc-empty">Nenhum extra selecionado ainda.<br />Use o menu à esquerda para personalizar.</div>
        )}
      ...
      <div className="orc-list" style={{ paddingBottom: 8 }}>
        <div className="orc-line">
          <span className="lbl">Piso</span>
          <span className="val" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <i style={{ width: 14, height: 14, borderRadius: 3, background: pisoCor?.hex, display: 'inline-block' }} />
            {pisoCor?.nome}
          </span>
        </div>
        <div className="orc-line">
          <span className="lbl">Paredes</span>
          <span className="val" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <i style={{ width: 14, height: 14, borderRadius: 3, background: paredeCor?.hex, display: 'inline-block' }} />
            {paredeCor?.nome}
          </span>
        </div>
      </div>
      ...
      <div className="resumo-actions">
        ...
        <p className="hint" style={{ margin: '2px 0 0' }}>
          Documento para o time comercial dar continuidade no atendimento pós-venda.
        </p>
      </div>
    </div>
  )
...
/* ---------- construção do briefing estruturado ---------- */
function construirBriefing(state, orcamento, cores) {
  const grupoPiso = PISOS[state.piso.grupo]?.rotulo
  const grupoParede = NAPAS[state.parede.grupo]?.rotulo
  const elet = {}
  for (const e of state.eletrica) elet[e.tipo] = (elet[e.tipo] || 0) + 1

  return {
    cliente: CLIENTE_DEMO.empresa,
    opcao: CLIENTE_DEMO.opcao,
    area: CLIENTE_DEMO.area,
    medidas: CLIENTE_DEMO.medidas,
    gerado_em: new Date().toLocaleString('pt-BR'),
    piso: { tipo: grupoPiso, cor: cores.pisoCor?.nome, codigo: cores.pisoCor?.cb, hex: cores.pisoCor?.hex },
    paredes: { tipo: grupoParede, cor: cores.paredeCor?.nome, codigo: cores.paredeCor?.cb, hex: cores.paredeCor?.hex },
    napa_madeira: { cor: cores.madeiraCor?.nome, hex: cores.madeiraCor?.hex },
    ...
    lonas_extra: state.extras.lonas,
...
function briefingHTML(b) {
  ...
  <div class="grid">
    ...
    <div><b>Paredes:</b> <span class="sw" style="background:${b.paredes.hex}"></span>${b.paredes.tipo} — ${b.paredes.cor} ${b.paredes.codigo || ''}</div>
    <div><b>Napa madeira:</b> <span class="sw" style="background:${b.napa_madeira.hex}"></span>${b.napa_madeira.cor}</div>
    <div><b>Painel de LED na testeira:</b> ${b.led_testeira ? 'SIM' : 'não'}</div>
  </div>
  ...
  <div class="grid">
    ...
    <div><b>Lonas extras:</b> ${b.lonas_extra}</div>
    <div><b>Logos extras:</b> ${b.logos_extra}</div>
  </div>

  <h2>Mobiliário adicional</h2>
  ${linhas(b.mobiliario_extra, [{ h: 'Item', k: 'item' }, { h: 'X (m)', k: 'x' }, { h: 'Z (m)', k: 'z' }])}
  ...
  <p class="muted">Coordenadas em metros a partir do canto fundo-esquerdo do stand (X = largura 0–10, Z = profundidade 0–4). Valores ilustrativos.</p>
  </body></html>`
}
